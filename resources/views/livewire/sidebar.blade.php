@php
    ...
    function navItem(bool $active): string {
        return $active
            ? 'group flex items-center p-2 text-gray-900 rounded-xl dark:text-gray-100 bg-gray-100 dark:bg-gray-700/80 border border-gray-200 dark:border-gray-700 shadow-sm transition-all'
            : 'group flex items-center p-2 text-gray-900 rounded-xl dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/70 transition-all';
    }
    function navSub(bool $active): string {
        return $active
            ? 'group flex items-center w-full rounded-lg px-3 py-1.5 text-sm font-medium text-gray-900 dark:text-gray-100 bg-gray-100 dark:bg-gray-700/70 border border-gray-200 dark:border-gray-700 transition-all'
            : 'group flex items-center w-full rounded-lg px-3 py-1.5 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:text-gray-900 dark:hover:text-white transition-all';
    }
    function navIcon(bool $active): string {
        return $active
            ? 'w-4 h-4 text-[#52ABB1]'
            : 'w-4 h-4 text-gray-500 dark:text-gray-400 group-hover:text-[#52ABB1] transition duration-200';
    }
    function navSection(): string {
        return 'px-3 mb-1 mt-3 text-[10px] font-semibold text-gray-400 uppercase tracking-[0.16em]';
    }
    ...
    <div class="flex h-full flex-col">
        ...
        <div class="border-b border-gray-200 dark:border-gray-700 px-4 py-2">
            <div class="rounded-xl border border-gray-200 dark:border-gray-700
                        bg-gray-50 dark:bg-gray-700/40 px-3 py-1.5">
                <div class="flex items-center justify-between gap-2">
                    <div>
                        <p class="text-[10px] text-gray-500 dark:text-gray-400">Acceso actual</p>
                        <p class="text-xs font-semibold text-gray-800 dark:text-gray-100">
                            {{ ucfirst($user->rol ?? 'Usuario') }}
                        </p>
                    </div>
                    <span class="inline-flex h-2 w-2 rounded-full bg-[#52ABB1]"></span>
                </div>
            </div>
        </div>
        ...
        <div
            id="sidebar-scroll"
            class="flex-1 overflow-y-auto px-3 py-3 custom-scroll"
            x-data
            x-init="
                const saved = localStorage.getItem('sb_scroll');
                if (saved) $el.scrollTop = parseInt(saved);
            "
            x-on:scroll="localStorage.setItem('sb_scroll', $el.scrollTop)"
        >
            ...
            <ul class="space-y-1">
                <li>
                    <a href="{{ route('dashboard') }}"
                       wire:navigate
                       class="{{ navItem(isActive('dashboard')) }}">
                        ...
                        <div class="ms-2.5 min-w-0">
                            <p class="text-sm font-semibold">Panel principal</p>
                            <p class="text-[10px] text-gray-400 dark:text-gray-500">Resumen operativo</p>
                        </div>
                    </a>
                </li>
            </ul>
            ...
            @if($isRecep || $isAdmin)
                ...
                <details class="group mb-1 rounded-xl border border-gray-200 dark:border-gray-700
                                bg-white dark:bg-gray-800 overflow-hidden"
                         {{ isActive('recepcion.pacientes.*') ? 'open' : '' }}>
                    <summary class="flex cursor-pointer list-none items-center justify-between
                                    gap-3 px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/40 transition">
                        <div class="flex items-center gap-2.5 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500">Registro y búsqueda</p>
                            </div>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 group-open:rotate-180"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </summary>
                    <div class="px-2 pb-1.5 space-y-0.5">
                        <a href="{{ route('recepcion.pacientes.index') }}" wire:navigate
                           class="{{ navSub(isActive('recepcion.pacientes.index')) }}">
                            <span class="mr-2 h-1.5 w-1.5 rounded-full bg-[#52ABB1]"></span>
                            Listado de pacientes
                        </a>
                        <a href="{{ route('recepcion.pacientes.create') }}" wire:navigate
                ...
                <details class="group mb-1 rounded-xl border border-gray-200 dark:border-gray-700
                    ...
                    <summary class="flex cursor-pointer list-none items-center justify-between
                        ...
                        <div class="flex items-center gap-2.5 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500">Admisión y prioridad</p>
                            </div>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 group-open:rotate-180"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    ...
                    <div class="px-2 pb-1.5 space-y-0.5">
                        <a href="{{ route('recepcion.ingresos.index') }}" wire:navigate
                           class="{{ navSub(isActive('recepcion.ingresos.index')) }}">
                            <span class="mr-2 h-1.5 w-1.5 rounded-full bg-[#52ABB1]"></span>
                            Historial de ingresos
                        </a>
                        <a href="{{ route('recepcion.ingresos.create') }}" wire:navigate
            ...
            @if($isNurse || $isAdmin)
                ...
                <details class="group mb-1 rounded-xl border border-gray-200 dark:border-gray-700
                    ...
                    <summary class="flex cursor-pointer list-none items-center justify-between
                        ...
                        <div class="flex items-center gap-2.5 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500">Clasificación y destino</p>
                            </div>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 group-open:rotate-180"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    ...
                    <div class="px-2 pb-1.5 space-y-0.5">
                        <a href="{{ route('enfermeria.index') }}" wire:navigate
                           class="{{ navSub(isActive('enfermeria.index')) }}">
                            <span class="mr-2 h-1.5 w-1.5 rounded-full bg-[#52ABB1]"></span>
                            Cola de enfermería
                        </a>
                        <a href="{{ route('enfermeria.pending') }}" wire:navigate
                    ...
                    <summary class="flex cursor-pointer list-none items-center justify-between
                        ...
                        <div class="flex items-center gap-2.5 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500">Colas por área</p>
                            </div>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 group-open:rotate-180"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    ...
                    <summary class="flex cursor-pointer list-none items-center justify-between
                        ...
                        <div class="flex items-center gap-2.5 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500">Consulta y seguimiento</p>
                            </div>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 group-open:rotate-180"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            ...
            @if($isLab || $isAdmin)
                ...
                <details class="group mb-1 rounded-xl border border-gray-200 dark:border-gray-700
                    ...
                    <summary class="flex cursor-pointer list-none items-center justify-between
                        ...
                        <div class="flex items-center gap-2.5 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500">Adjuntos PDF</p>
                            </div>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 group-open:rotate-180"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    ...
                    <div class="px-2 pb-1.5 space-y-0.5">
                        <a href="{{ route('laboratorio.index') }}" wire:navigate
            ...
            @if($isAdmin)
                ...
                <details class="group mb-1 rounded-xl border border-gray-200 dark:border-gray-700
                                bg-white dark:bg-gray-800 overflow-hidden"
                         {{ isActive(['admin.staff.*','admin.areas.*','admin.examenes.*']) ? 'open' : '' }}>
                    <summary class="flex cursor-pointer list-none items-center justify-between
                                    gap-3 px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/40 transition">
                        <div class="flex items-center gap-2.5 min-w-0">
                            <svg class="{{ navIcon(isActive(['admin.staff.*','admin.areas.*','admin.examenes.*'])) }}"
                                 fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[10px] text-gray-400 dark:text-gray-500">Configuración base</p>
                            </div>
                        </div>
                        <svg class="w-3.5 h-3.5 text-gray-400 transition-transform duration-200 group-open:rotate-180"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    ...
                    <div class="px-2 pb-1.5 space-y-0.5">
                        ...
                        <a href="{{ route('admin.areas.index') }}" wire:navigate
                           class="{{ navSub(isActive('admin.areas.*')) }}">
                            <span class="mr-2 h-1.5 w-1.5 rounded-full bg-[#52ABB1]"></span>Áreas
                        </a>
                        <a href="{{ route('admin.examenes.index') }}" wire:navigate
   class="{{ navSub(isActive('admin.examenes.*')) }}">
    <span class="mr-2 h-1.5 w-1.5 rounded-full bg-[#52ABB1]"></span>Exámenes de laboratorio
</a>
                    </div>
                </details>
            @endif
...
<div class="border-t border-gray-200 dark:border-gray-700 px-4 py-2">
    <div class="rounded-xl bg-gray-50 dark:bg-gray-700/40
                border border-gray-200 dark:border-gray-700 px-3 py-2">
        <p class="text-[10px] text-gray-500 dark:text-gray-400">Sesión iniciada como</p>
        <p class="truncate text-xs font-semibold text-gray-800 dark:text-gray-100">
            {{ collect(explode(' ', trim($user->name ?? '')))->first() }}
